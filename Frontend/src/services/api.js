...
export async function explainQuestion({ question, student_id, session_id, token, subject, class_id, clarification_context }) {
  const response = await fetch('/api/explain', {
    ...
    body: JSON.stringify({ question, student_id, session_id, subject, class_id, clarification_context })
  });
  if (!response.ok) {
...
export async function updateCoursePrerequisite(courseName, relationshipId, relationship_type, token) {
  const res = await fetch(`/api/courses/${courseName}/prerequisites/${relationshipId}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.error || 'Failed to update prerequisite');
  }
  return await res.json();
}
